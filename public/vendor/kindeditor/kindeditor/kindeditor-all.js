...
KindEditor.plugin('filemanager', function(K) {
	var self = this, name = 'filemanager',
		fileManagerJson = K.undef(self.fileManagerJson, self.basePath + 'php/file_manager_jsont.php'),
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function bindEvent(el, result, data, createFunc) {
			...
			if (data.is_dir) {
				el.children().eq(0).click(function(e) {   //el.click(function(e) {
		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				var data = fileList[i],
				/*新增*/
				AltTxt=data.is_dir?"打开文件夹：":"引用文件：",
				AltIco=data.is_dir?"open.png":"quote.png", //区分文件与文件夹引用与打开图标
				IsDirYN=data.is_dir?"dir":"file", //区分文件与文件夹参数传递
				CurFileUrl=K.formatUrl(result.current_url+data.filename, 'absolute'),
				QuoteIcon="<img class='QuoteBtn' src='"+imgPath+AltIco+"' Style='width:30px;height:30px;position:absolute;top:35px;right:35px;display:none;' alt='"+AltTxt+CurFileUrl+"' title='"+AltTxt+CurFileUrl+"' />", //定义引用按钮样式
				DeleteIcon="<img class='kdedt_delete' src='"+imgPath+"delete.png' Style='width:30px;height:30px;position:absolute;top:-10px;right:-10px;display:none;' alt='删除："+CurFileUrl+"' title='删除："+CurFileUrl+"' DeUrl='"+CurFileUrl+"' IsFrd='"+IsDirYN+"' />",
				/*新增*/
				div = K('<div class="ke-inline-block ke-item"></div>');
				bodyDiv.append(div);
				var photoDiv = K('<div class="ke-inline-block ke-photo">'+QuoteIcon+DeleteIcon+'</div>')   //var photoDiv = K('<div class="ke-inline-block ke-photo"></div>')
...
KindEditor.plugin('flash', function(K) {
	var self = this, name = 'flash', lang = self.lang(name + '.'),
		...
		uploadJson = K.undef(self.uploadJson, self.basePath + 'php/upload_jsont.php');
	self.plugin.flash = {
		edit : function() {
			var html = [
